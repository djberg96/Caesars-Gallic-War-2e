...
Whenever the matrix calls for you to activate Germania, but all German units have been eliminated, withdrawn, or are simply too weak to attack then activate Vercingetorix instead if he's on the board.
...
If there is no minor leader on the board, then follow the instructions of the matrix. If no instruction is explicitly provided then use the card as a political action that targets a randomly selected Roman controlled tribe.
