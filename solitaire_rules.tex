...
\begin{enumerate}
  ...
  \begin{enumerate}[leftmargin=0in]
    ...
    \item If the card revealed is a Minor Revolt, then roll a die. On a 3 or less then activate Germania as per \ref{solitaire:germania}. Otherwise, select a tribe random randomly using the Random Tribe Selection table. Continue to re-roll if the tribe is already controlled by the bot, eliminated, or subjugated. Then, activate that tribe as per \ref{solitaire:tribal}.
  ...
  \begin{itemize}
    ...
    \item The bot will reinforce a bot controlled area with up to two friendly Gallic or German units that are adjacent and not themselves under attack, unless the original defending units are outnumbered by 3-1 or more.
    
    \item If there is more than one area from which the bot could defend, then randomly determine which area the reinforcements come from. If there happen to be more than two blocks in that area, then randomly determin which two reinforce the battle while leaving the others behind.
