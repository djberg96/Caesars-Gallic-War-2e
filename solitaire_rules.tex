...
\begin{enumerate}
  ...
  If the card revealed is a Barbarian controlled area but all tribes of that area were eliminated earlier in the turn, or the Germania card and the Germans have been eliminated or left via Strategic Withdrawal, then use the Random Tribe Selection table for the target of a political action. Re-roll if the result happens to match the current card.
  ...
  \begin{itemize}
    ...
    \item If the bot is attacking more than one area, resolve the area with the most units attacking + reinforcing first. If there is a tie, resolve the order randomly.
  \end{itemize}
